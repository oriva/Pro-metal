...
$('.this-is-mebel button').click(function () {
  ...
  $('.mebel-form__select').html('');
  var allPic = $(this).parents().eq(2).find('img').data('img').split(';');
  allPic.forEach(function(value, key){
    if(key==0){
      $('.mebel-form__select').append('<img src="'+value+'" class="selected">');
    } else {
      $('.mebel-form__select').append('<img src="'+value+'">');
    }
  });
});
$('.this-is-mebel img').click(function () {
  $('.mebel-form').addClass('show');
  $('.mebel-form__name').html($(this).parents().eq(1).find('span.name').html());
  $('.mebel-name').val($(this).parents().eq(1).find('span.name').html());
  $('.mebel-form__price-data').html($(this).parents().eq(1).find('.price-data').html());
  $('.mebel-price').val($(this).parents().eq(1).find('.price-data').html());
  $('.mebel-form__pic').attr('src', '../img/1px.jpg');
  $('.mebel-form__pic').attr('src', $(this).attr('src'));
  $('.mebel-form__select').html('');
  var allPic = $(this).data('img').split(';');
  allPic.forEach(function(value, key){
    if(key==0){
      $('.mebel-form__select').append('<img src="'+value+'" class="selected">');
    } else {
...
$('.mebel-form show').click(function(e){
  if(e.target!==$('.mebel-form__main')){
